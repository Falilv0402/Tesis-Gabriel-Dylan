...
## FASE 18 — Auditoría completa de las 35 HU/CP contra el código real (2026-09-11)

A pedido explícito del cliente ("quiero una redacción impecable reflejando todo lo que tenemos ahora mismo"), se auditó cada HU y CP contra el código real (frontend, backend y migraciones) — no solo las que ya se sabía que estaban mal, sino las 35 completas. Se usaron 2 sub-agentes de exploración en paralelo (auth/usuarios/auditoría; estudiante/intervenciones) mientras se revisaba directamente el resto (dashboard, datos, modelo).

- [x] **3 HU más eliminadas** (además de las 5 de la Fase 17) por describir funcionalidad hoy inalcanzable o inexistente:
  - HU011 "ejecutar el modelo con un solo botón" — era el botón "Reentrenar" de `ModeloView` (EM2022), pestaña ausente del menú hoy.
  - HU017 "explicación simple del riesgo en lenguaje natural" — no existe ninguna frase generada para colegio propio; es la MISMA función que HU012 (`factoresRiesgoColegio`), sin capacidad adicional real.
  - HU031 "ajustar los parámetros del modelo predictivo" (sliders de umbral) — mismo `ModeloView` muerto; colegio propio usa umbrales fijos del entrenamiento (70%/45%), sin pantalla para tocarlos.
- [x] **19 HU reescritas** por describir algo que no corresponde al comportamiento real verificado. Los hallazgos más significativos:
  - **HU001 (registro)**: el rol real es Director/Coordinador, NO Administrador (el propio formulario lo dice); ya no hay campo "distrito"; las 5 reglas de contraseña son concretas; el mensaje de correo duplicado es literal; y en el caso más común el registro deja al usuario autenticado de inmediato (auto-login), no "redirige a login".
  - **HU006 (auditoría)**: el `audit_log` registra 19 tipos de acción reales (sesión, usuarios, modelo, intervenciones, anotaciones, exportaciones, configuración), no solo "accesos"; existe un panel real con búsqueda y filtro por fecha, visible solo para admin/superadmin — Director no tiene acceso, ni siquiera a nivel de RLS.
  - **HU019 (alertas)** y **HU020 (recomendaciones)**: la mecánica estaba mal descrita. Las alertas son un correo automático SOLO tras reentrenar, cuando aparecen alumnos NUEVOS en ALTO (no una alerta genérica "por canal configurado"); las "recomendaciones" nunca se muestran al usuario — son un texto de respaldo oculto (descripción por defecto de una intervención vacía, o el cuerpo del correo de alerta), casi siempre genérico para colegio propio.
  - **HU028/HU029/HU030 (datos)**: usaban lenguaje genérico de la época EM2022 ("formato CSV/XLSX" cuando solo se acepta .xlsx/.xls; "corrige inconsistencias" cuando en realidad omite archivos problemáticos y reporta por qué; "filas con errores" cuando son hojas/archivos completos).
  - **HU032 (entrenar modelo)**: describía una pantalla "Re-entrenar Modelo" separada que no existe — hoy subir el Excel entrena automáticamente, sin un botón aparte.
  - El resto (HU002, HU004, HU005, HU009, HU012-HU016, HU018, HU021) recibieron correcciones de precisión: mensajes exactos, KPIs exactos, límites reales (ej. 25 alumnos en el ranking), y quitar escenarios que en realidad duplicaban el estado vacío ya documentado en otra HU (ej. "sin modelo propio" solo vive en HU010, no se repite en cada HU del dashboard).
- [x] **13 HU quedaron intactas**, ya verificadas como precisas: HU003, HU007, HU008, HU010, HU022-HU027, HU033-HU035.
- [x] Resultado: **32 HU / 71 CP**, renumerados y verificados con `assert` (cada HU con tantos CP como escenarios, sin huecos ni duplicados, pestañas == lista). Las CP conservadas se copiaron tal cual (mismo formato, solo su ID cambia); las de HU reescritas se redactaron de cero.
- [x] Entregables: `P20261012_Historias de Usuario y Criterios de Validacion v1.7.xlsx` y `P20261012_Casos de Prueba v1.6.xlsx` (`docs/excel/`), generados por `docs/auditar_hu_cp_v3.py`.
...
## FASE 19 — Flujo de aprobación de planes: Coordinador propone, Director aprueba (2026-09-12)

Pedido explícito de Mathias: hasta ahora Director y Coordinador tenían exactamente los mismos permisos académicos (la única diferencia real era que Director administra el rol de su equipo, 0016), y la palabra "equipo" que ya aparecía en HU022 (notificaciones) no tenía ningún lugar propio en la app.

- [x] **Tabla `plan_estado` nueva** (0017): una fila por alumno (no versionada -- agregar hitos no reinicia el estado, se mantiene simple), con estado `borrador → en_revision → aprobado/rechazado`. RLS aplica la regla central: Director o Coordinador pueden enviar/reenviar a revisión, pero **solo Director** (o admin/superadmin) puede aprobar o rechazar.
- [x] **UI**: en el detalle del alumno (pestaña Plan), un banner de estado con "Enviar a revisión" (Coordinador/Director) y "Aprobar"/"Pedir cambios" (solo Director, con comentario obligatorio para pedir cambios). La pestaña "Usuarios" se renombra a **"Equipo"** solo para Director (Coordinador nunca la ve) y ahí aparece la cola "Planes a revisión" con aprobación de un clic.
- [x] **Notificaciones reales**: enviar a revisión avisa a todo el equipo del colegio (reusa HU022); aprobar/rechazar notifica puntualmente a quien lo envió (función nueva `notificarDirigida`, no al equipo completo).
- [x] **2 bugs reales encontrados y corregidos verificando esto en vivo** (0018): el `codigo_ie` de un alumno (tal como lo sirve el modelo del colegio, ej. "0831305") no siempre coincide en formato con `profiles.codigo_ie` del usuario (ej. "831305") -- mismo desfase de padding ya visto antes en el proyecto.
  1. Las políticas RLS de `plan_estado` comparaban con igualdad estricta -- "Enviar a revisión" fallaba con `42501`. Corregido normalizando con `ltrim(..., '0')` en ambos lados.
  2. **Hallazgo más importante**: la MISMA falla ya afectaba a `notificaciones` (0013) desde que esa tabla existe -- agendar un hito o anotación insertaba la notificación con el codigo_ie del alumno, y la política la rechazaba en silencio (best-effort) contra el codigo_ie del perfil. **HU022 (notificaciones de equipo) no estaba funcionando de verdad para ninguna cuenta en esta situación**, aunque el código se veía correcto y así se documentó en la Fase 18. Corregido con el mismo `ltrim`.
  3. Bug adicional del lado del cliente: `loadPlanesPendientes` comparaba `codigo_ie` por texto (`.or(...)`) sin probar la variante con cero inicial, dejando la cola de "Planes a revisión" siempre vacía. Se quitó ese filtro -- la política RLS ya normalizada es la única fuente de verdad de qué filas son del colegio del Director.
- [x] **Verificado end-to-end en producción** con 2 cuentas de prueba reales (Director + Coordinador del mismo colegio, Trapiche IE 831305): Coordinador agrega hitos y envía a revisión → Director recibe la notificación de equipo, ve la cola en "Equipo", aprueba un plan de un clic (notifica al Coordinador) y rechaza otro con comentario desde el detalle del alumno (notifica al Coordinador con el comentario) → el plan rechazado vuelve a mostrar "Enviar a revisión". Sin errores de consola en ningún paso.
- [x] **Diseño pulido**: el banner de estado y la cola "Planes a revisión" usaban estilos inline sin sombra/hover — se movieron a clases CSS propias (`globals.css`) que sí siguen el sistema de diseño ya establecido (`var(--shadow-*)`, transiciones, tarjetas con borde izquierdo + hover, mismo patrón que `.alert-card`). Verificado visualmente en producción con cuenta real: los 4 estados (borrador/en revisión/aprobado/rechazado) y la cola se ven consistentes con el resto de la app.
- [x] **HU nueva agregada** ("Como Coordinador, quiero enviar el plan de seguimiento de un alumno a revisión de mi Director...") con sus 3 CP, al final de EP04 (Priorización e Intervención, que pasa de 4 a 5 HU). Resultado: **33 HU / 74 CP**, verificado con `assert`.
- [x] Entregables: `P20261012_Historias de Usuario y Criterios de Validacion v1.8.xlsx` y `P20261012_Casos de Prueba v1.7.xlsx` (`docs/excel/`), generados por `docs/agregar_hu_plan_revision.py`.
- [ ] **Pendiente**: borrar del alumno real los hitos de prueba creados durante la verificación (Mori Romayna, Sandoval Garcia, Paredes Herrera) y las filas de `plan_estado`/`notificaciones` de prueba, y las cuentas de prueba acumuladas (`testpdf@upc.edu.pe`, `test.respaldo@upc.edu.pe`, `test.coord.plan@upc.edu.pe`, `test.ui.review@upc.edu.pe`).
- [x] **Cerrado el hueco de "Anotaciones" y "Plan de hitos"**: se agregaron sus 2 HU (con 2 CP cada una) en EP04, entre "Notificaciones de equipo" y "Enviar plan a revisión" (orden narrativo). Resultado: **35 HU / 78 CP**.
- [x] **Bug real encontrado y corregido al redactar la HU de hitos**: la UI (`canEditAll`) ya dejaba a un Director marcar como completado cualquier hito de su equipo, consistente con la política RLS `owner_or_director_update_hitos` (0006_coordinador_role.sql) que existe exactamente para eso — pero el propio código del cliente (`toggleMilestone`, `useStudents.ts`) agregaba `.eq("autor_id", session.id)` a la consulta, así que ese `UPDATE` nunca afectaba filas ajenas: 0 filas, sin error, y la casilla se revertía en el siguiente refresco sin que el Director entendiera por qué. Se quitó ese filtro de más y se agregó `.select()` para detectar el caso de 0 filas como error real.
- [x] Entregables: `P20261012_Historias de Usuario y Criterios de Validacion v1.9.xlsx` y `P20261012_Casos de Prueba v1.8.xlsx` (`docs/excel/`), generados por `docs/agregar_hu_anotaciones_hitos.py`.

## FASE 20 — Endurecimiento de seguridad de cara a la auditoría externa (2026-09-14)

Pedido explícito de Mathias: la app va a ser auditada por externos (requisito del curso de sus clientes) que "harán bastantes pruebas" — probablemente pegándole directo a las APIs, no solo a través de la UI. Revisión de seguridad puntual, no una auditoría exhaustiva de todo el esquema.

- [x] **Mensaje de recomendación genérico (HU018)**: `recommendation()` (`format.ts`) solo reconocía las categorías de riesgo de EM2022 — ninguna de las 5 categorías que genera `tipoRiesgoColegio()` para colegio propio coincidía, así que el 100% de los alumnos ALTO de colegio propio caían al mensaje genérico ("Monitoreo académico continuo..."), tanto en pantalla como en el correo de alerta al equipo. Se agregaron ramas específicas para las 4 categorías de colegio propio alcanzables en ese punto de la función.
- [x] **Hallazgo de seguridad — endpoints de lectura sin autenticación**: `GET /v1/colegio/{codigo_ie}/predicciones|resumen|respaldo` no exigían sesión — devuelven nombre completo, salón y nivel/probabilidad de riesgo de alumnos reales (menores de edad), y `codigo_ie` es el código MINEDU del colegio, público y adivinable/enumerable. El frontend tampoco enviaba token en esas llamadas. Corregido reutilizando `require_admin_de_colegio` (ya existía para los endpoints de escritura) también en los 3 GET — ahora exige el mismo JWT de Supabase + verificación de que el perfil pertenezca a ESE colegio (o sea superadmin). Frontend actualizado para enviar `Authorization: Bearer <access_token>` en las 6 llamadas a `/v1/colegio/*` (`useColegio.ts`, `useColegioModels.ts`, `useAdmin.ts`).
- [x] **Hallazgo de seguridad — fuga de segregación de datos en RLS (0019)**: mismo patrón que ya se había corregido en `profiles`/`audit_log` (0009) e `intervenciones` (0014), pero `anotaciones` y `plan_hitos` habían quedado afuera de esas dos rondas. Sus políticas de SELECT eran `auth.role() = 'authenticated'` desde el inicio (0002/0004) — cualquier Director/Coordinador logueado podía leer, vía la API REST de Supabase directamente (sin pasar por la UI), las anotaciones privadas y los hitos de plan de **cualquier** colegio, no solo el suyo. De paso se encontró que las políticas de UPDATE (`owner_or_director_update_anotaciones`/`_hitos`, 0006/0007) dejaban a cualquier Director editar filas de un colegio ajeno, y la de DELETE de anotaciones a cualquier 'admin' borrar notas ajenas — mismo defecto, rol válido pero sin verificar colegio. Migración `0019_scope_anotaciones_hitos_rls.sql` agrega `codigo_ie`/`distrito` a ambas tablas (poblados por trigger desde el perfil del autor, mismo patrón que 0014) y scopea las 4 políticas de cada tabla. **Pendiente que Mathias la corra en el SQL Editor de Supabase.**
- [x] **Rate limiting agregado** (`slowapi`): límite global de 120/min por IP (vía middleware, corre antes de cualquier dependencia de auth/API key — protege incluso los intentos que fallan la autenticación) más límites propios de 5/min en `/reentrenamiento` y `/procesar`, 10/min en `/restaurar`. Verificado con `TestClient` (429 al 6.º intento en la misma ventana) y en producción real contra `api.satraapp.com`.
- [ ] **Pendiente / fuera de alcance**: no se auditaron `estudiantes`/`notas_periodos` (tablas de 0001_init, parecen sin uso) ni `predicciones` (solo snapshots del dataset EM2022 anonimizado, sin nombres).
- [x] **SEO + Open Graph** (2026-09-21): metadata de `layout.tsx` ampliada (title/description reescritos, keywords, canonical, robots, openGraph, twitter card), imagen `opengraph-image.png` (1200x630) nueva con la paleta real de marca, `sitemap.ts`/`robots.ts` (SATRA es una SPA de una sola ruta pública). Favicon (`icon.png`/`apple-icon.png`) recortado a solo el escudo — antes era el logo completo y el texto "SATRA" quedaba ilegible a 16-32px.
- [x] **Anotaciones y plan de hitos en tiempo real**: se agregó suscripción `postgres_changes` (patrón calcado de `useNotificaciones.ts`) en `useAnnotations.ts` y `useStudents.ts`, filtrada por `estudiante_id`. De paso se descubrió que `supabase_realtime` tenía **0 tablas** — es decir, ni siquiera la campanita de notificaciones estaba recibiendo push real (solo se actualizaba al montar/navegar). Mathias activó `notificaciones`, `anotaciones` y `plan_hitos` en Database > Publications. Verificado en vivo con dos pestañas del navegador sobre el mismo alumno (Mori Romayna, IE 831305, cuenta `test.realtime1@upc.edu.pe`): tanto una anotación como un hito nuevos aparecieron en la segunda pestaña sin recargar.
- [ ] **Pendiente**: sumar a la limpieza de datos de prueba la cuenta `test.realtime1@upc.edu.pe` (Director, IE 831305) y la anotación/hito de prueba creados en Mori Romayna durante esta verificación.

## FASE 21 — Corrección de incidencias reportadas sobre HU/CP (2026-09-24)

Revisión externa reportó 18 incidencias sobre el excel de Historias de Usuario y el de Casos de Prueba (tracker tipo Redmine); se corrigieron las 10 primeras (las 8 restantes no llegaron a compartirse — Mathias indicó avanzar revisando el repo directamente, ya que las HU/CP las formuló y cambió el propio equipo).

- [x] **HU018**: el criterio de aceptación decía "un texto de regla fija según el riesgo" sin definir cuál — se enumera la regla completa (BAJO/MEDIO/ALTO con o sin área específica), igual a `recommendation()` en `format.ts`. De paso ya no describe el bug viejo (mensaje genérico por defecto para colegio propio, cerrado en FASE 20).
- [x] **HU019**: el escenario "Registro exitoso" mezclaba dos comportamientos (descripción con texto / descripción vacía) en un mismo resultado, y referenciaba mal la regla fija (decía HU020, es HU018). Se separa la responsabilidad: este escenario solo describe el guardado; qué texto se usa si está vacía es 100% competencia de HU018.
- [x] **HU021 y HU022**: el resultado documentaba literalmente la fuga de RLS que existía antes de la migración 0019 ("visible para cualquier usuario autenticado, no solo el autor"). Se corrige a "visible para ese mismo equipo (cualquier Director o Coordinador de la misma IE, o superadmin)". HU022 no estaba entre las 10 incidencias reportadas, pero tiene el mismo defecto exacto que HU021 — se corrigió de una vez.
- [x] **Autor de los CP**: el campo tenía el rol bajo prueba (ej. "Coordinador Académico") en las 78 pestañas del documento, no solo en las 3 reportadas (CP024/027/029) — confirmado revisando una muestra al azar antes de decidir el alcance. A pedido de Mathias, se reemplaza por **Gabriel Torres** en las 78.
- [x] **CP024/028/029/030**: precondiciones sin datos concretos (alumno/colegio genérico) — ahora referencian un colegio real (IE 831305) o especifican la data reproducible exacta a preparar.
- [x] **CP025**: "backend desconectado" no decía cómo reproducirlo — se agrega el comando concreto (`docker compose stop backend`).
- [x] **CP026**: los pasos describían un flujo viejo ("Predecir alumnos en riesgo" + formulario de filtros + "obtener respuesta") que no corresponde al flujo real de HU010-1 (el dashboard muestra los niveles de riesgo de inmediato, sin ese paso manual) — reescritos para que coincidan.
- [x] **CP042/CP044** (mismos ajustes que HU018/HU019) y **CP048/CP050** (mismo ajuste que HU021/HU022) actualizados para que la documentación quede consistente de punta a punta.
- [x] Scripts `docs/corregir_incidencias_hu_cp.py` (aplica los cambios) y `docs/verificar_correcciones_hu_cp.py` (11 asserts: contenido corregido, ninguna fila/pestaña perdida, diff exacto contra la versión anterior confirmado — solo cambiaron las celdas esperadas, nada más). Ambos corrieron limpio.
- [x] Entregables: `P20261012_Historias de Usuario y Criterios de Validacion v2.0.xlsx` y `P20261012_Casos de Prueba v1.9.xlsx` (`docs/excel/`).
- [ ] **Pendiente**: las 8 incidencias restantes del tracker (11-18/18) — hace falta verlas para saber si tocan HU/CP que no se revisaron en esta pasada.
