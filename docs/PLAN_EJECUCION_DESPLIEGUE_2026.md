...
- [ ] **Pendiente de decidir con Mathias**: subir esto a producción (push a `main` → Vercel redeploya el frontend solo; el backend en Hetzner necesita el mismo proceso manual de `scp` + `docker compose up -d --build backend` que la vez anterior, por el fix de `prediction_service.py`).
