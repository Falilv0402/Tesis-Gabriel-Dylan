...
- [ ] **Pendiente / fuera de alcance de esta pasada**: no se auditaron `estudiantes`/`notas_periodos` (tablas de 0001_init, parecen sin uso — ningún hook las consulta) ni `predicciones` (sí se usa, pero solo guarda snapshots del dataset EM2022 anonimizado, sin nombres) ni rate limiting en el backend FastAPI (no hay ninguno hoy). Si la auditoría incluye pruebas de fuerza bruta/enumeración, valdría la pena revisarlo.
